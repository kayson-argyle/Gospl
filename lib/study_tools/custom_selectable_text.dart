...
Kayson Argyle created this file.
This is a custom version of Selectable text, it will probably break at some point.
It is an exact copy of selectable text, but it requires that a TextSpanEditingController is passed into the constructor, thereby giving control over what is selected.
SelectableText.rich doesn't work because at the moment I don't need it to
If it breaks, start the process over by copying the definition of SelectableText in the flutter framework and requiring that a TextSpanEditingController is passed into the constructor.
This is very hacky and there is probably a better way to do this but I'm lazy
Cheers!
...
class TextSpanEditingController extends TextEditingController {
  TextSpanEditingController({required TextSpan text}):
    _textSpan = text,
    super(text: text.toPlainText(includeSemanticsLabels: false));
  ...
  void giveEditableTextKey(GlobalKey<EditableTextState> key) {
    editableTextKey = key;
  }
...
class _CustomSelectableTextState extends State<CustomSelectableText> implements TextSelectionGestureDetectorBuilderDelegate {
  ...
  late final TextSpanEditingController _controller = widget.controller;
  ...
  void initState() {
    ...
    _controller.giveEditableTextKey(editableTextKey);
  }
